...
import { pluginAcervo } from './src/servidor/pluginAcervo.js'
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      // O acervo não é proxiado: ele é servido aqui mesmo, pelas mesmas rotas
      // que o `server.js` monta. Sem isto, `/api/acervo` não existe sob o
      // `npm run dev` — 404 com a `base` padrão, `index.html` com `BASE_PATH=/`
      // — e o README promete que dev e Railway se comportam igual.
      pluginAcervo(),
    ],
    server: {
