...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      // O acervo e a cota não são proxiados: são servidos aqui mesmo, pelas
      // mesmas rotas que o `server.js` monta, e a JSearch ganha o mesmo
      // middleware de contagem. Sem isto, `/api/acervo` e `/api/cota` não
      // existem sob o `npm run dev` — 404 com a `base` padrão, `index.html`
      // com `BASE_PATH=/` — e o README promete que dev e Railway se
      // comportam igual.
      pluginServidor(),
    ],
    server: {
